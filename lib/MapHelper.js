...
class MapHelper {
  ...
  constructor (burl,durl) {
      ...
      this.newMarker=null//地物编辑-marker
      this.map = L.map('map', {
      crs: L.CRS.EPSG4326,
      center: [38, 119],
      maxZoom: 18,
    ...
    that.map.on(L.Draw.Event.CREATED, function (e) {
      ...
      switch (that.key) {
        ...
        case 2:
          ...
          break;
        case 3:
          that.newMarker=new L.circleMarker(layer._latlng)
          if (confirm("yes?")){
            that.commit();
...
// 创建任意几何范围查询实例
  ...
  */
  ...
  commit(){
    this.newMarker =this.newMarker.toGeoJSON();
    this.newMarker.properties = {POP: 1, CAPITAL: 'test'};
